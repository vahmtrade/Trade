...
class Economy:
    ...
    def read_index(self):
        ########## Read pmi data #############
        service_pmi=pd.read_excel(f'{DB}/forex_database/{self.country}.xlsx',sheet_name='service pmi')
        try:
            for i in service_pmi.index:
                if type(service_pmi.loc[i,'date'])==str:
                    service_pmi.loc[i,'date']=pd.to_datetime(service_pmi.loc[i,'date'][:10])
        ...
        try:
            manufacturing_pmi=pd.read_excel(f'{DB}/forex_database/{self.country}.xlsx',sheet_name='manufacturing pmi')
            for i in manufacturing_pmi.index:
                if type(manufacturing_pmi.loc[i,'date'])==str:
                    manufacturing_pmi.loc[i,'date']=pd.to_datetime(manufacturing_pmi.loc[i,'date'][:10])
        ...
        ########### Read I.R data #############
        try:
            ir=pd.read_excel(f'{DB}/forex_database/{self.country}.xlsx',sheet_name='ir')
        ...
        #GDP Q o Q
        try:
            gdp_q=pd.read_excel(f'{DB}/forex_database/{self.country}.xlsx',sheet_name='gdp qoq')
            # chnge type of data to datetime
        ...
        #GDP Y o Y
        try:
            gdp_y=pd.read_excel(f'{DB}/forex_database/{self.country}.xlsx',sheet_name='gdp yoy')
            # chnge type of data to datetime
        ...
        try:
            unemployment=pd.read_excel(f'{DB}/forex_database/{self.country}.xlsx',sheet_name='unemployment rate')
            # chnge type of data to datetime
            for i in unemployment.index:
                if type(unemployment.loc[i,'date'])==str:
                    unemployment.loc[i,'date']=pd.to_datetime(unemployment.loc[i,'date'][:10])
        ...
        #cpi_mom
        try:
            cpi_m=pd.read_excel(f'{DB}/forex_database/{self.country}.xlsx',sheet_name='cpi mom')
                # chnge type of data to datetime
        ...
        #cpi_yoy
        try:
            cpi_y=pd.read_excel(f'{DB}/forex_database/{self.country}.xlsx',sheet_name='cpi yoy')
                # chnge type of data to datetime
        ...
        #ppi_mom
        try:
            ppi_m=pd.read_excel(f'{DB}/forex_database/{self.country}.xlsx',sheet_name='ppi mom')
                # chnge type of data to datetime
        ...
        #ppi_yoy
        try:
            ppi_y=pd.read_excel(f'{DB}/forex_database/{self.country}.xlsx',sheet_name='ppi yoy')
                # chnge type of data to datetime
    ...
    def plot_economy(self):
        ...
        plt.axvline(self.manufacturing_pmi['actual'].median(),color='black',linestyle='dashed')
